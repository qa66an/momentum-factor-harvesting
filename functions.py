...
def get_fundementals_for_dates(df, sf1_db_path, meta_data_path):
    ...
    SELECT
        ...
        (
            ...
            AND s.calendardate <= CAST(p.momentum_end AS DATE)
            ORDER BY s.calendardate DESC
            LIMIT 1
        ) AS shares_momentum_end,

        (
            SELECT s.shareswa
            FROM sf1db.sf1_core s
            WHERE s.ticker = p.ticker
            AND s.calendardate <= CAST(p.buy_date AS DATE)
            ORDER BY s.calendardate DESC
            LIMIT 1
        ) AS shares_buy_date,
                    (
    ...
    result["mc_momentum_end"] = result["shares_momentum_end"] * result["end_price"]
...
def create_top_and_bottom_deciles(file, top_decile_path, bottom_decile_path):
    ...
    df = df[(df["mc_momentum_end"] > limit_min)]
    df = df.sort_values(
        by=["buy_date", "momentum_pct"], ascending=[True, False]
    ).reset_index(drop=True)
    df = df[df["momentum_pct"] < df["momentum_pct"].quantile(0.99)]
