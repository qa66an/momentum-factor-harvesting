...
if __name__ == "__main__":
    ...
    for year, dates_list in dates.items():
        ...
        for d in dates_list:
            print(f"\nProcessing date {d}...")
            mom_start, mom_end, buy_date, sell_date = get_momentum_and_holding_dates(
                d, trading_days
            )
            df = get_four_date_prices(
                sep_db_path, mom_start, mom_end, buy_date, sell_date
            )
        ...
        final_year_df.to_csv(
            f"./{raw_yearly_data_path}/momentum_12-1_1m_monthly_sep_{year}.csv",
            index=False,
        )
        print(
            f"\n✅ Done. Saved to ./{raw_yearly_data_path}/momentum_12-1_1m_monthly_sep_{year}.csv"
        )
